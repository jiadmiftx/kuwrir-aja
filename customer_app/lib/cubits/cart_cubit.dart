...
class CartState {
  ...
  double get packagingFeeTotal => items.fold(0, (sum, i) => sum + i.packagingFeeTotal);
...
class CartCubit extends Cubit<CartState> {
  ...
  void addItem(
    ...
    if (state.merchantId != null && state.merchantId != merchantId) {
      emit(CartState(
        items: [CartItem(product: product, quantity: quantity, selectedVariants: selectedVariants)],
    ...
    final existing = state.items
        .indexWhere((i) => i.product.id == product.id && i.variantKey == key);
    if (existing >= 0) {
      ...
      emit(state.copyWith(
        items: [
          ...state.items,
          CartItem(product: product, quantity: quantity, selectedVariants: selectedVariants),
        ],
        merchantId: merchantId,
        merchantName: merchantName,
      ));
  ...
  void removeItem(String productId, {String variantKey = ''}) {
    final updated = state.items
        .where((i) => !(i.product.id == productId && i.variantKey == variantKey))
        .toList();
    emit(state.copyWith(items: updated));
  }
  ...
  void decrementItem(String productId, {String variantKey = ''}) {
    final idx = state.items
        .indexWhere((i) => i.product.id == productId && i.variantKey == variantKey);
    if (idx < 0) return;
